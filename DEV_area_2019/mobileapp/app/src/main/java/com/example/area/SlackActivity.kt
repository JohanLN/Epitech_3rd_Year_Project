...
/**
 * Class SlackActivity. Activity class, where the user can sign to the Slack service and set all Actions Reactions settings.
 */
...
class SlackActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loginBouton = findViewById(R.id.login_button_yt)
        logoutBouton = findViewById(R.id.logout_button_yt)
        ...
        loginBouton?.setOnClickListener {
            oauth!!.popup(provider, OAuthCallback {
                ...
                updateUI(true)
                Toast.makeText(this@SlackActivity, "You are sign in !", Toast.LENGTH_SHORT).show()
                //registerService(data.token)
                ApiRequest().registerService("Slack",data.token, progressBar, applicationContext)
            })
        }
        logoutBouton?.setOnClickListener {
            updateUI(false)
            Toast.makeText(this@SlackActivity, "You are sign out !", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun initialize() {
        progressBar = findViewById(R.id.SlProgress)
