...
/**
 * Class ImgurActivity. Activity class, where the user can sign to the Imgur service and set all Actions Reactions settings.
 */
...
class ImgurActivity : AppCompatActivity() {
    ...
    /**
     * Display the activity, allows the user to sign to the Imgur service and to set Actions Reactions settings.
     */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loginBouton = findViewById(R.id.login_button_gm)
        logoutBouton = findViewById(R.id.logout_button_gm)
        ...
        loginBouton?.setOnClickListener {
            oauth!!.popup(provider, OAuthCallback {
                ...
                //////// ACCESS TOKEN ////////
                data ->
                Log.w(TAG, data.token)
                updateUI(true)
                Toast.makeText(this@ImgurActivity, "You are sign in !", Toast.LENGTH_SHORT).show()
                //registerService(data.token)
                ApiRequest().registerService("Imgur", data.token, progressBar, applicationContext)

            })
        }
        logoutBouton?.setOnClickListener {
            updateUI(false)
            Toast.makeText(this@ImgurActivity, "You are sign out !", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun initialize() {
        progressBar = findViewById(R.id.ImProgress)
